...
@interface MonsterJaktViewController ()
{
    struct{
        CGFloat red;
        CGFloat green;
...
@implementation MonsterJaktViewController
...
-(void)setupCamera
{
    ...
    */
    ...
    filteredVideoView.backgroundColor = [UIColor clearColor];

    //[filteredVideoView addSubview:_dValues];
    
    //setup the monster Box
    if(![[_gameManager monsterByIdentifier:3] isCatched]){
        UIImage *monsterImage = [_gameManager getMonsterImage:3];
    ...
    if(![[_gameManager monsterByIdentifier:2] isCatched]){
        UIImage *monsterImage = [_gameManager getMonsterImage:2];
        CGRect frame = CGRectMake(100, 100, 80, 80);
...
-(void)updateView
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        */
        ...
        if(![_monsterBox isHidden] && [[_gameManager monsterByIdentifier:3] isCatched]){
        ...
        } else if (![_monsterBox2 isHidden] && [[_gameManager monsterByIdentifier:2] isCatched]){
            [UIView animateWithDuration:2.0
                             animations:^{
                                 //[_monsterBox2 setHidden:YES];
...
-(void)captureMonster:(UIButton *)sender
{
    ...
    if(![_monsterBox isHidden] && _mobFocusSq)
    {
        ...
        {
            //isCaptured = YES;
            [_gameManager setMonster:3 catched:YES];
            NSLog(@"catched 3");
        ...
        {
            //isCaptured = YES;
            [_gameManager setMonster:2 catched:YES];
            NSLog(@"catched 2");
            [UIView animateWithDuration:3.0
                             animations:^{
                                 [captureLabel setAlpha:1.0];
...
-(NSUInteger)getColorMonster
{
    ...
    } else if (trackValues.green > 0.5 && trackValues.blue < 0.4){ // green
